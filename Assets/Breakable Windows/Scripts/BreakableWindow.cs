...
public class BreakableWindow : MonoBehaviour {
    ...
    private float brokenWindowTimer = 5f;
    private GameObject[] brokenSplinters;
    ...
    private void Update()
    {
        if (isWindowBroken)
        {
            if (brokenWindowTimer > 0)
            {
                brokenWindowTimer -= Time.deltaTime;
            }
            else
            {
                foreach (GameObject s in brokenSplinters)
                    Destroy(s);
            }
        }
    }
